...
export default class WebClient {
    ...
    getUserAgent() {
        ...
        return `${config.crawler.name}/${config.crawler.version} ${hasContact
    ...
    clearCache(duration) {
        ...
        const result = this.database.query("DELETE FROM metadata WHERE cacheDate < ?", [cutoff]);
        return {
            result,
            cutoff
        };
    ...
    async isAllowed(url) {
        try {
            ...
            if (!result)
                log.crawler.warn(`${config.crawler.name} is not allowed to crawl ${url}`);
            return result;
        }
        catch (error) {
            log.crawler.error(error?.code || error?.message);
