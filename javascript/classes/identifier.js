...
export default class Identifier {
    ...
    generate(type) {
        ...
        let attempts = 0;
        const allowedChars = Array.from(Array.from({ length: 128 }, (_, i) => String.fromCharCode(i))
            .filter((c) => identifier.regex.test(c)));
        const createIdentifier = () => Array.from({ length: identifier.length }, () => allowedChars[Math.floor(Math.random() * allowedChars.length)]).join("");
        while (attempts < maxRetries) {
            const id = createIdentifier();
            const rows = this.database.query("SELECT * FROM identifiers WHERE id = ? AND type = ?", [id, type]).rows;
            if (!rows || rows.length === 0) {
                this.database.query("INSERT INTO identifiers (id, type) VALUES (?, ?)", [id, type]);
     ...
     */
    get(id) {
        const rows = this.database.query("SELECT type FROM identifiers WHERE id = ?", [id]).rows;
        return rows?.[0]?.type;
    }
}
